...
let StewApp = {
  ...
  setupContentPolicy: function() {
   ({
      ...
      _isBlocker: function(host, path) {
        let paths = Blockers[host];
        if (!paths) return false;
        for (let i in paths) {
          if (paths[i] == "/") return true;
          else if (path.startsWith(paths[i])) return true;
        }
        return false;
      },
      ...
      shouldLoad: function(contentType, contentLocation, requestOrigin, context, mimeTypeGuess, extra) {
        try {
          ...
          if (pledgeInfo.pledged &&
              !pledgeInfo.unblocked &&
              (this._isBlocker(trackerDomain, contentLocation.path) ||
               this._isBlocker(contentLocation.host, contentLocation.path))
             ) {
