...
let TopDomains = {
  ...
  getPackagedDomains: function() {
    let scriptLoader = Cc["@mozilla.org/moz/jssubscript-loader;1"].getService(Ci.mozIJSSubScriptLoader);
    scriptLoader.loadSubScript(data.url("packagedDomains.js"), null, "UTF-8");

    // Sort domains within each locale by rank
    let sortedDomains = {};
    Object.keys(packagedDomains).forEach(locale => {
      sortedDomains[locale] = {};
      Object.keys(packagedDomains[locale]).sort((a,b) => {
        return packagedDomains[locale][a].rank - packagedDomains[locale][b].rank;
      }).forEach(domain => {
        sortedDomains[locale][domain] = packagedDomains[locale][domain];
      });
    });

    this.getPackagedDomains = function() sortedDomains;
    return this.getPackagedDomains();
  },

  getParticipatingDomains: function() {
    ...
    let domains = this.getPackagedDomains()[locale];
    let sites = Object.keys(domains);
    for (var i = 0; i < sites.length; i++) {
      if (i % 3 != 0 || domains[sites[i]].category == "Blocked") {
        delete domains[sites[i]];
      }
    }

    this.getParticipatingDomains = _ => domains;
    return domains;
  },
